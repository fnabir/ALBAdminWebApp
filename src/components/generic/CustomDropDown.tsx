  ...
  return (
    <div className={className}>
      <div className={"relative mt-4"}>
        ...
        <select id={id}
                className={`w-full py-2 rounded-lg border` + (disabled ? "bg-gray-700" : "text-white bg-transparent") + " " +
                  ...
                  "    focus:border-blue-400 focus:outline-none focus:ring-0 peer" }
          ...
          {options.map((option) => (
            <option className={"bg-slate-700 text-white"} key={option.label ? option.label : option.value}
      ...
      <div
        className={helperText ? ((color == "error" ? "text-red-400" : "text-sky-300") + " flex mt-1 ml-1 text-xs") : "hidden"}>
        {helperText}
      </div>
    </div>
  );
