...
import CardTitleDetails from "@/components/card/CardTitleDetails";
...
export default function ErrorNice3000() {
  ...
  if (!loading && !user) return router.push("login")
    ...
    return (
      <Layout 
        pageTitle="NICE 3000 | Asian Lift Bangladesh"
        headerTitle="Error Code | NICE 3000">
        <div>
          <div className="flex items-center m-2 gap-x-2">
            <label className="text-wrap text-xl">Error Code</label>
            <input className="block w-40 rounded-md border-0 p-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-xl sm:leading-6"
              value={errorCode} onChange={(e) => setErrorCode(e.target.value)} required/>
            <button className="rounded-md w-28 bg-blue-600 px-3 py-1.5 text font-semibold leading-6 text-white shadow-sm hover:bg-blue-500" onClick={handleSearch}>Search</button>
          </div>
          {
            description == "" ? (
              <div></div>
            ) : description == "No Error Code Found!" ? (
              <CardIcon title={"Error"} subtitle={"No Error Code Found!"}>
